...
const actions = {};
const prefix = '/wd/hub/session/:sessionId/';
...
actions.doubleTap = function(action) {
  ...
  return post.call(this, 'element/' + element + '/doubleTap', {
    x: action.x || 0,
    y: action.y || 0
  });
};
...
actions.press = function(action) {
  ...
  return post.call(this, 'element/' + element + '/touchAndHold', {
    duration: action.duration || 1
  });
};
...
actions.drag = function(action) {
  ...
  return post.call(this, 'target/' + element + '/dragfromtoforduration', {
    toX: action.toX,
    toY: action.toY,
    duration: action.duration || 1
  });
