...
IOS.prototype.startDevice = function *(caps) {
  ...
  yield _.retry(co.wrap(function *() {
    logger.debug('Trying to start wda server...')
    self.xctest.stop();
    yield self.device.uninstall(WDA_BUNDLE_ID);
...
IOS.prototype.startSimulator = function *() {
  ...
  if (this.args.reuse && isBooted) {
    if (this.args.reuse === 1) {
      yield this.device.uninstall(this.bundleId);
    }
    if (this.args.reuse === 3) {
      const ifAppExist = yield this.device.exists(this.bundleId);
      if (ifAppExist) {
        this.args.app = '';
      }
    }
      ...
      this.device
        ...
        .then(isBooted => {
          if (isBooted) {
            ...
            reject(new Error(`Simulator ${this.device.deviceId} is not booted.`));
          }
        });
    });
  }, 3000, 10);
